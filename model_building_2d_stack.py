...
def create_model_2D_stack(input_size, output_size):

    model = Sequential()
    model.add(Input(input_size))
    model.add(Dropout(0.2))
    model.add(Conv2D(16, (5, 3), activation='tanh'))
...
if __name__ == "__main__":
    ...
    X, y = load_training_data(num_sample=1000, x_dimension=x_dimension, img_resize_factor=img_resize_factor,
                              shrinkx=True, stack=True)
    input_size, output_size = X.shape[1:], y.shape[1]

    modelname = model_namer(dimension=x_dimension, num_sample=len(X), sub_sample=5, fmc_scaler=1.75e-14,
                           img_resize=img_resize_factor, remark='stack-2d', epochs=epochs, version=3)

    print(f'The model name is {modelname}')
    # Build model.
    model = create_model_2D_stack(input_size, output_size)
    print(model.summary())
